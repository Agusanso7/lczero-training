...
V6_STRUCT_STRING = '288s224s' #'4si7432s832sBBBBBBBbfffffffffffffffIHH4H'
...
class ChunkParserInner:
    ...
    def convert_v6_to_tuple(self, content):
        ...
        (planes, move) = self.v6_struct.unpack(content)
        """
        ...
        probs = np.unpackbits(np.frombuffer(move, dtype=np.uint8)).astype(np.float32)

        # Concatenate all byteplanes. Make the last plane all 1's so the NN can
        # detect edges of the board more easily
        #planes = planes.tobytes() + self.flat_planes[1]
    ...
    def parse(self):
        ...
        gen = self.tuple_gen(gen)  # convert v6->tuple # TODO: change this !!
